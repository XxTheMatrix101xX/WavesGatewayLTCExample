...
class KoreChainQueryService(gw.ChainQueryService):
    """
    Implementation of an ChainQueryService on the Kore Blockchain.
    """
    ...
    def _extract_receivers(self, transaction: dict) -> List[gw.TransactionReceiver]:
        """Extracts the receivers of an unparsed KORE transaction."""
        ...
        for vout in transaction['vout']:
            ...
            for address in vout['scriptPubKey']['addresses']:
                kore_transaction_receiver = gw.TransactionReceiver(address=address, amount=vout['value'])
                results.append(kore_transaction_receiver)
    ...
    def _resolve_senders(self, transaction: dict) -> List[gw.TransactionSender]:
        """Extracts the senders of an unparsed KORE transaction"""
        ...
        for vin in transaction['vin']:
            ...
            vin_transaction_raw = self._kore_proxy.getrawtransaction(vin['txid'])
            vin_transaction = self._kore_proxy.decoderawtransaction(vin_transaction_raw)
            ...
            for address in vin_transaction['vout'][vin['vout']]['scriptPubKey']['addresses']:
                kore_transaction_sender = gw.TransactionSender(address=address)
                results.append(kore_transaction_sender)

        return self._filter_sender_duplicates(results)

    def get_transaction(self, tx: str) -> gw.Transaction:
        raw_transaction = self._kore_proxy.getrawtransaction(tx)
        transaction = self._kore_proxy.decoderawtransaction(raw_transaction)
    ...
    def get_transactions_of_block_at_height(self, height: gw.CoinBlockHeight) -> List[gw.Transaction]:
        block_hash = self._kore_proxy.getblockhash(height)
        block = self._kore_proxy.getblock(block_hash)

        get_transaction_tasks = pool.Pool()  # kore server does not accept more than one parallel connection
